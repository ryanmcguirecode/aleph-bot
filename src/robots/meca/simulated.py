...
from pyvista import AmbiguousDataError
...
class MecaSim(Meca, Robot):
    config_class = MecaConfig
    name = "mecasim"

    def __init__(self, config: MecaConfig) -> None:
        ...
        self.robot: Any = self.scene.add_entity(
            gs.morphs.URDF(
                file="src/robots/meca/meca500r3.urdf",
                pos=(0, 0, 0),  # Position the MECA robot above the plane,
                euler=(0, 0, 0),
                fixed=True,
                collision=False,
            ),
    ...
    def connect(self, calibrate: bool = True) -> None:
        ...
        dofs_idx = [joint.dof_idx_local for joint in movable_joints]
    ...
    def get_ee_pose(self) -> EndEffectorPose:
        ...
        pos = self.robot.get_link("meca_axis_6_link").get_pos().cpu().numpy()
        quat = self.robot.get_link("meca_axis_6_link").get_quat().cpu().numpy()
