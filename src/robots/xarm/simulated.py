...
from robots.xarm7.config import XArm7Config
from robots.joints import JointVelocities
from robots.xarm7.robot import XArm7
...
KP = np.array([253.0, 214.0, 557.0, 200.0, 1500.0, 900.0, 100.0])
KV = np.array([11.0, 14.0, 20.0, 12.0, 50.0, 22.0, 5.0])


class XArm7Sim(XArm7, Robot):
    config_class = XArm7Config
    name = "xarm7sim"

    def __init__(self, config: XArm7Config) -> None:
        ...
        self.robot: Any = self.scene.add_entity(
            gs.morphs.URDF(
                file="src/robots/xarm7/xarm7.urdf",
                pos=(0, 0, 0),  # Position the XArm7 robot above the plane
                fixed=True,
                collision=False,
            ),
        )

        self.lite6 = self.scene.add_entity(
            gs.morphs.URDF(
                file="src/robots/lite6/lite6.urdf",
                pos=(0.5, 0, 0),  # Position the Lite6 robot above the plane
                fixed=True,
                collision=False,
            ),
        )

        self.fairino = self.scene.add_entity(
            gs.morphs.URDF(
                file="src/robots/fairinofr3/fairino3_v6.urdf",
                pos=(-0.5, 0, 0),  # Position the Fairino robot above the plane
                fixed=True,
                collision=False,
            ),
        )
    ...
    def get_ee_pose(self) -> EndEffectorPose:
        ...
        pos = self.robot.get_link("link7").get_pos().cpu().numpy()
        quat = self.robot.get_link("link7").get_quat().cpu().numpy()
