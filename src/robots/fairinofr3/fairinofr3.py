...
class Fairino:
    ...
    REST_JOINTS: Final[tuple[float, float, float, float, float, float, float]] = (
        0,
        0,
        0,
        0,
    ...
    def __init__(self, config: FairinoConfig):
        ...
        self.kin = Kinematics(
            urdf_path="src/robots/fairinofr3/fairino3_v6.urdf",
            package_dir="src/robots/fairinofr3",
            tool_axis="z",
            tool_length=0.148,
            ee_link="wrist3_link",
        )
    ...
    def delta_to_velocity(self, delta: EndEffectorDelta) -> JointVelocities:
        ...
        rot_deltas = rearranged[3:]  # Convert deg to rad

        damping = 1e-4 / (
            1 + (self.get_joint_positions()[4])
        )  # Damping based on joint 5 proximity to singularity
